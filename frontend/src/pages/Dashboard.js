...
//  1. navBrand text: "Orphanage Manager" → "Hope Orphanage"
//  2. Footer: already "2026" — unchanged
//  3. "About Us" nav link wired to onNavigate('about')
//  4. "Contact" nav link wired to onNavigate('contact')
...
function Dashboard({ user, onLogout, onNavigate }) {
  ...
  const statCards = [
    { label: 'Activities & Events', value: loadingStats ? '...' : (stats?.events ?? 0),                                               icon: '📅', color: '#4a90e2' },
    { label: 'Children',            value: loadingStats ? '...' : (stats?.children ?? 0),                                             icon: '👧', color: '#e67e22' },
    { label: 'Total Donations',     value: loadingStats ? '...' : `₹${Number(stats?.totalAmount ?? 0).toLocaleString('en-IN')}`,      icon: '💰', color: '#27ae60' },
    { label: 'Applications',        value: loadingStats ? '...' : (stats?.applications ?? 0),                                         icon: '📋', color: '#8e44ad' },
  ];

  const sections = [
    { name: 'Children',     icon: '👧', desc: 'Track children admitted, their details and status.',  action: () => onNavigate('children') },
    { name: 'Donations',    icon: '💰', desc: 'Monitor incoming donations and donor information.',    action: () => onNavigate('donations') },
    { name: 'Applications', icon: '📋', desc: 'Submit a donation or an adoption application.',        action: () => setAppMenu(prev => !prev) },
  ];
  ...
  return (
    <div style={styles.page}>
      ...
      <nav style={styles.navbar}>
        ...
        {/* Brand name — CHANGED to "Hope Orphanage" */}
      ...
      <main style={styles.main}>
        ...
        <p style={styles.pageSubtitle}>Welcome back! Here's a live summary of the orphanage.</p>
        ...
        <div style={styles.tableBox}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={styles.table}>
              ...
              <tbody>
                {loadingActivity ? (
                  <tr><td colSpan="4" style={{ ...styles.td, textAlign: 'center', color: '#aaa' }}>Loading activity...</td></tr>
                ) : activity.length === 0 ? (
                  <tr><td colSpan="4" style={{ ...styles.td, textAlign: 'center', color: '#aaa' }}>No activity yet.</td></tr>
                ) : (
                  activity.map((row, i) => (
                ...
                <>
                  ...
                  {appMenu && (
                    <div style={styles.subMenu}>
                      <button style={{ ...styles.subBtn, backgroundColor: '#27ae60' }} onClick={() => onNavigate('donate')}>💰 Make a Donation</button>
                      <button style={{ ...styles.subBtn, backgroundColor: '#8e44ad' }} onClick={() => onNavigate('adopt')}>👶 Apply to Adopt</button>
                    </div>
                  )}
                </>
              ) : (
                <button style={styles.panelBtn} onClick={sec.action}>View {sec.name}</button>
              )}
            </div>
          ))}
        </div>
